...
def send_verification_code(request):
    ...
    if last_sent and timezone.now().timestamp() - float(last_sent) < 30:
    ...
    try:
        ...
        return JsonResponse({'success': False, 'message': 'Mail server error. Please contact support.'})
...
def verify_code(request):
    ...
    if not stored_code_hash or not stored_email:
        return JsonResponse({'success': False, 'message': 'No code found. Request a new one.'})

    if email and email != stored_email:
        return JsonResponse({'success': False, 'message': 'Verification email does not match.'})

    if timezone.now().timestamp() > float(expiry):
        return JsonResponse({'success': False, 'message': 'Code expired. Please try again.'})
